package ServerObjectInterfaceApp;


/**
* ServerObjectInterfaceApp/ServerObjectInterfacePOA.java .
* Generated by the IDL-to-Java compiler (portable), version "3.2"
* from C:/rafi/distributed-system-project/src/ServerObjectInterface.idl
* Monday, March 4, 2019 9:05:49 o'clock PM EST
*/

public abstract class ServerObjectInterfacePOA extends org.omg.PortableServer.Servant
 implements ServerObjectInterfaceApp.ServerObjectInterfaceOperations, org.omg.CORBA.portable.InvokeHandler
{

  // Constructors

  private static java.util.Hashtable _methods = new java.util.Hashtable ();
  static
  {
    _methods.put ("addItem", new java.lang.Integer (0));
    _methods.put ("listItem", new java.lang.Integer (1));
    _methods.put ("removeItem", new java.lang.Integer (2));
    _methods.put ("borrowItem", new java.lang.Integer (3));
    _methods.put ("findItem", new java.lang.Integer (4));
    _methods.put ("returnItem", new java.lang.Integer (5));
    _methods.put ("waitingList", new java.lang.Integer (6));
    _methods.put ("findBorrowedItems", new java.lang.Integer (7));
    _methods.put ("findWaitingItems", new java.lang.Integer (8));
    _methods.put ("exchangeItem", new java.lang.Integer (9));
    _methods.put ("shutdown", new java.lang.Integer (10));
  }

  public org.omg.CORBA.portable.OutputStream _invoke (String $method,
                                org.omg.CORBA.portable.InputStream in,
                                org.omg.CORBA.portable.ResponseHandler $rh)
  {
    org.omg.CORBA.portable.OutputStream out = null;
    java.lang.Integer __method = (java.lang.Integer)_methods.get ($method);
    if (__method == null)
      throw new org.omg.CORBA.BAD_OPERATION (0, org.omg.CORBA.CompletionStatus.COMPLETED_MAYBE);

    switch (__method.intValue ())
    {
       case 0:  // ServerObjectInterfaceApp/ServerObjectInterface/addItem
       {
         String managerID = in.read_string ();
         String itemID = in.read_string ();
         String itemName = in.read_string ();
         int quantity = in.read_long ();
         boolean $result = false;
         $result = this.addItem (managerID, itemID, itemName, quantity);
         out = $rh.createReply();
         out.write_boolean ($result);
         break;
       }

       case 1:  // ServerObjectInterfaceApp/ServerObjectInterface/listItem
       {
         String managerID = in.read_string ();
         String $result = null;
         $result = this.listItem (managerID);
         out = $rh.createReply();
         out.write_string ($result);
         break;
       }

       case 2:  // ServerObjectInterfaceApp/ServerObjectInterface/removeItem
       {
         String managerID = in.read_string ();
         String itemID = in.read_string ();
         int quantity = in.read_long ();
         boolean $result = false;
         $result = this.removeItem (managerID, itemID, quantity);
         out = $rh.createReply();
         out.write_boolean ($result);
         break;
       }

       case 3:  // ServerObjectInterfaceApp/ServerObjectInterface/borrowItem
       {
         String userID = in.read_string ();
         String itemID = in.read_string ();
         int numberOfDay = in.read_long ();
         boolean $result = false;
         $result = this.borrowItem (userID, itemID, numberOfDay);
         out = $rh.createReply();
         out.write_boolean ($result);
         break;
       }

       case 4:  // ServerObjectInterfaceApp/ServerObjectInterface/findItem
       {
         String userID = in.read_string ();
         String itemName = in.read_string ();
         String $result = null;
         $result = this.findItem (userID, itemName);
         out = $rh.createReply();
         out.write_string ($result);
         break;
       }

       case 5:  // ServerObjectInterfaceApp/ServerObjectInterface/returnItem
       {
         String userID = in.read_string ();
         String itemID = in.read_string ();
         boolean $result = false;
         $result = this.returnItem (userID, itemID);
         out = $rh.createReply();
         out.write_boolean ($result);
         break;
       }

       case 6:  // ServerObjectInterfaceApp/ServerObjectInterface/waitingList
       {
         String userID = in.read_string ();
         String itemID = in.read_string ();
         boolean $result = false;
         $result = this.waitingList (userID, itemID);
         out = $rh.createReply();
         out.write_boolean ($result);
         break;
       }

       case 7:  // ServerObjectInterfaceApp/ServerObjectInterface/findBorrowedItems
       {
         String userId = in.read_string ();
         String $result = null;
         $result = this.findBorrowedItems (userId);
         out = $rh.createReply();
         out.write_string ($result);
         break;
       }

       case 8:  // ServerObjectInterfaceApp/ServerObjectInterface/findWaitingItems
       {
         String $result = null;
         $result = this.findWaitingItems ();
         out = $rh.createReply();
         out.write_string ($result);
         break;
       }

       case 9:  // ServerObjectInterfaceApp/ServerObjectInterface/exchangeItem
       {
         String userID = in.read_string ();
         String newItemID = in.read_string ();
         String oldItemID = in.read_string ();
         boolean $result = false;
         $result = this.exchangeItem (userID, newItemID, oldItemID);
         out = $rh.createReply();
         out.write_boolean ($result);
         break;
       }

       case 10:  // ServerObjectInterfaceApp/ServerObjectInterface/shutdown
       {
         this.shutdown ();
         out = $rh.createReply();
         break;
       }

       default:
         throw new org.omg.CORBA.BAD_OPERATION (0, org.omg.CORBA.CompletionStatus.COMPLETED_MAYBE);
    }

    return out;
  } // _invoke

  // Type-specific CORBA::Object operations
  private static String[] __ids = {
    "IDL:ServerObjectInterfaceApp/ServerObjectInterface:1.0"};

  public String[] _all_interfaces (org.omg.PortableServer.POA poa, byte[] objectId)
  {
    return (String[])__ids.clone ();
  }

  public ServerObjectInterface _this() 
  {
    return ServerObjectInterfaceHelper.narrow(
    super._this_object());
  }

  public ServerObjectInterface _this(org.omg.CORBA.ORB orb) 
  {
    return ServerObjectInterfaceHelper.narrow(
    super._this_object(orb));
  }


} // class ServerObjectInterfacePOA
